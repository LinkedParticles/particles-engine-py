...
| `memory consolidate` | **The dream cycle**: one scheduled verb that runs the cross-session maintenance passes in order: extract catch-up (PENDING snapshots, oldest first, capped at `consolidation.max_pending_entries`), the reconcile sweep, the capped + delta-scoped contradiction/duplicate census (one `collect_cards` pass), the curation-queue refresh over that same collection, utility mining, and the `MEMORY.md` projection re-render. It then writes one `CONSOLIDATION_RUN` operator event (per-pass status/durations/LLM call counts, machine-readable census, `completed_at` = the next run's delta watermark) and renders a delta report ("+2 contradictions since last run"). Cadence is launchd/cron, not a daemon: `--if-due` exits 0 unless the last successful run is older than `consolidation.min_interval_hours`, so over-scheduling is harmless. `--structural-only` (or no API key / open breaker / `consolidation.semantic: false`) still runs the LLM-free passes with every skip disclosed; a degraded contradiction line reads "not probed this run", never "0". `--scope delta\|store`, `--output FILE`, `--format markdown\|json`. Exit codes: 0 success/skip, 1 pass(es) failed, 2 could not start. One cycle at a time via a stale-reclaimed `consolidate.lock`. `particles audit` records the same run event (`actor: audit`), joining the delta chain. See [Operator Guide → Scheduled consolidation](operator-guide/scheduled-consolidation.md). |
...
| `corpus delete <entry-id>` | Delete a corpus entry, its snapshots, and every particle whose only source is that entry. A particle that another entry also supports is kept, with this entry's source refs removed; if one of them was its earliest source, the next-earliest becomes its age anchor. Premise refs on derived particles are left for the revalidation ladder. The preview shows the delete-versus-keep split before the confirmation prompt, and the delete records a `CORPUS_ENTRY_DELETED` operator event holding the entry id and counts, not the deleted content. It also sweeps the index rows keyed on the deleted particles (`particle_subjects`, `particle_tag_edges`, `particle_relations`), removes subjects left with no remaining link, and drops `synthesis_cache` rows keyed on those now-orphaned subjects. `--yes` skips the confirmation prompt. |
...
| `benchmark memory` | Run the **agent-memory system benchmark**: LongMemEval against the whole pipeline (deposit → extract → reconcile → query), not a single extractor (its own `benchmark` group, deliberately not under `extractor`). Reports four conditions in two labeled families: retrieval-stage Recall@k / Precision@k scored through provenance chains, and end-to-end QA accuracy for `qa_particles` vs the `qa_full_context` / `qa_no_memory` baselines under one pinned answer model (`llm.benchmark_answer`; judge on `llm.benchmark`). Estimate/confirm cost gate (`--estimate`, `--yes`); dataset downloaded on demand at a pinned revision with SHA-256 verification. Supports `--limit N`/`--all`, `--variant oracle\|s\|m`, `--types`, `--format table\|json`, `--output`, `--store-dir`, `--dataset-file`. See the [benchmarks documentation](benchmarks.md). |
| `benchmark observer` | **The two-project observer fixture** (gate B): two repositories' memory files, sharing generic subjects, evolve over `--days` and are harvested into one scratch store through the real pipeline with scripted extraction and a scripted contradiction probe (zero LLM calls). Each day every line a project currently states is checked through that project's observer: in view, or retired by cross-project supersession, the generation cascade, or a surviving particle attested only by the other project (`active_elsewhere`, the lens's own cost). `--seed` (repeatable; default 1–8), `--days` (14), `--output`, `--format markdown\|json`, `--store-dir` keeps the scratch stores. The number `claude_code.observer_scope`'s default is decided against; write-up in [Benchmarks → Project observer scope](benchmarks/observer-scope.md). |
| `benchmark relevance-floor` | Measure the **query relevance floor on real questions**: how often the gate that skips the answer step refuses a question the store could have answered. `harvest` builds a private held-out set from your own agent transcripts (explicit memory queries plus typed questions, tagged apart, secrets redacted). The bare verb replays it against a store and reports the refusal curve over floors 0.10–0.40 with **no LLM call**; `--judge` answers each question with the gate disabled, labels the answer grounded-and-useful or not, and sweeps the answerable-but-refused / unanswerable-but-passed rates under both conditionings. Estimate/confirm cost gate (`--estimate`, `--yes`), priced from each question's real retrieved context; `--replay-from` reuses a saved replay; `resweep` re-renders a saved report over any floors for free. The table carries no question text; the held-out set and JSON report do, and are refused inside a git work tree without `--allow-in-repo`. Supports `--limit N`, `--top-k`, `--store`, `--format table\|json`, `--output`, `--checkpoint`. See the [relevance-floor results](benchmarks/relevance-floor.md). |
...
- **Maps the frontmatter onto deposit fields.** `source` (fallback `url`) becomes
  ...
  is the record of what was seen; `mutability=STABLE`, `fetch_policy=NEVER`),
  which means corpus-link-gap lint, mention reconciliation, and refetch identity
...
- **Stamps `SOURCE_TYPE = WEB_PAGE`**, so the entry is now trustable, decayable
...
clause of the adoption story: swap in one edit (`particles memory serve`),
onboard in one command, **bring your history with you**.

```bash
# First, rehearse: print what the import would produce, and what it would
# drop, without depositing or writing anything (no store is opened).
uv run particles import mcp-memory ~/.mcp/memory.jsonl --dry-run
...
### Rehearsing an import: `--dry-run`

An import runs on a user's real graph, and a wrong mapping is otherwise found
only after particles are written (recoverable by `reindex`, since the export is
in the corpus, but a poor first impression). `--dry-run` parses the export, runs
**the same mapping function the extractor runs**, and reports:

- record counts in the export's own vocabulary (entities, observations,
  relations), then the Subjects and particles they map to;
- **entities with no observations**, split into those that would not survive at
  all and those that survive as a relation endpoint (with their entity type);
- everything dropped from the particles and preserved in the blob: skipped
  lines, non-text observations, and fields the reference format does not
  define;
- a sample of mapped records, spread across the export (`--sample N`).

It opens no session and deposits nothing, so it needs no `db init`, is safe on a
populated store, and (unlike the import itself) runs in remote mode. The counts
are what the export contributes: the dry run does not consult the store, so
Subject re-attachment and duplicate-claim dedup can only make the real import
smaller. `--json` emits the same report as JSON; the exit code is 1 when nothing
in the export would become a particle.
...
- **An entity with no observations and no relations is not migrated.** The
  reference server keeps an entity that has never been observed; a particle
  store has no belief to hang it on, so nothing is created. This is accepted
  permanently rather than worked around: a bare Subject minted by the importer
  is a `PHANTOM_SUBJECT` finding on the next `particles lint`. The verb names
  the affected entities as it runs, and the count is repeated as an extraction
  quality note. An observation-less entity that is a **relation endpoint** does
  migrate, with its `entityType`. To keep an isolated one, recreate it through
  the façade's `create_entities` after the swap.
...
bumps are additive-compat per strict SemVer, so your existing store keeps
